...
if (isset($_POST['submit'])) {
  ...
  if ($validateTool->validateRequiredStringField($firstName) && $validateTool->characterLimiter($firstName, 100)) {
    ...
    if ($validateTool->validateRequiredStringField($lastName) && $validateTool->characterLimiter($lastName, 100)) {
      ...
      if ($validateTool->validateRequiredField($password) && $validateTool->characterLimiter($password, 100)) {
  ...
  if ($isValidFirstName && $isValidLastName && $isValidPassword && $isValidMinor && $isValidPortfolio && $isValidGoals && $isValidHometown && $isValidHobbies && $isValidSecondary && $isValidLinkedIn && $isValidCEmail) {
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    try {
      $stmt = $conn->prepare("UPDATE student_info_2020 SET student_first_name = :firstName, student_last_name = :lastName, student_major = :major, student_minor = :minor,
        student_portfolio = :portfolio, student_linkedin = :linkedIn, student_secondary = :secondary, student_hometown = :hometown, student_career_goals = :goals,
        student_hobbies = :hobbies, student_state = :state, student_password = :password, student_contact_email = :cemail WHERE student_id = :studentid");
...
try {
  ...
  foreach($statement->fetchAll() as $k=>$v) {
    $firstName = $v['student_first_name'];
    $lastName = $v['student_last_name'];
    $major = $v['student_major'];
    $minor = $v['student_minor'];
    $portfolio = $v['student_portfolio'];
    $secondary = $v['student_secondary'];
    $linkedIn = $v['student_linkedin'];
    $goals = $v['student_career_goals'];
    $hometown = $v['student_hometown'];
    $state = $v['student_state'];
    $hobbies = $v['student_hobbies'];
    $cEmail = $v['student_contact_email'];
  ...
  <body>
    <div class="container">
      <div class="fContainer">
        <form action="studentUpdate.php" method="POST" id="input">
          <header id="inputHeader">
            <h1>Welcome Students!</h1>
            <p>Here you can update your information. Once correctly submitted you will be redirected to the login page.</p>
          </header>
          ...
          <div class="row1">
            ...
            <input type="text" name="fName" value="<?php echo $firstName;?>" tabindex="1" id="firstName"></input>
            <p id="error"><?php echo $errorMessageLastName; ?></p>
            <label for="lName">Last Name:</label>
            <input type="text" name="lName" value="<?php echo $lastName;?>" tabindex="2" id="lastName"></input>
          </div>
          <div class="row2">
            <p id="error"><?php echo $errorMessagePassword; ?></p>
            <label for="password">Password:</label>
            <input type="password" name="password" id="password" value="<?php echo $password;?>" tabindex="3"></input><img id="show" src="assets/eye.svg" title="Show Password" alt="Show Password" height="25px" width="25px">
